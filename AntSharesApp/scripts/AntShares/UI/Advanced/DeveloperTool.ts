namespace AntShares.UI.Advanced
{
    export class DeveloperTool extends TabBase {
        ...
        private OnGetHeightClick = () => {
            ...
            debugLog(1);
            Global.RestClient.getHeight().then(obj => {
                debugLog(obj);
                debugLog(obj.height);
            });
        }
        ...
        private OnGetBlockClick = () => {
            ...
            debugLog(2);
            Global.RestClient.getBlock(1).then(block => {
                debugLog(block);
        ...
        private OnGetTransactionClick = () => {
            ...
            Global.RestClient.getTx(txid).then(tx => {
                debugLog(tx);
                debugLog(tx.txid);
                //let asset = <Core.RegisterTransaction>tx;
                debugLog(tx.asset);
                debugLog(tx.asset.name);
                debugLog(tx.asset.name[0].name);
                debugLog(this.getName(tx.asset.name[0].name));
            });
        }
        ...
        private OnGetAddressClick = () => {
            ...
            Global.RestClient.getAddr(addr).then(addr => {
                debugLog(addr);
                let utxo = JSON.stringify(addr.utxo);
                //debugLog(utxo);

                let strBalances: string = JSON.stringify(addr.balances);
        ...
        private OnPostBroadcastClick = () => {
            ...
            let strTx = "";
            ...
            Core.SignatureContext.create(tx, "AntShares.Core." + Core.TransactionType[tx.type]).then(result => {
                ...
                //let x = context.signatures;
                //let y = x[0].get("02c9fc2026614721177900883280645413cd8cdd4778a9bf7eb483b9ccf7535250");
                //let sig: string = new Uint8Array(y).toHexString();
                //debugLog(sig);
